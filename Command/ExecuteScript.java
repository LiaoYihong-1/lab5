...
public class ExecuteScript extends AbstractCommand {
     ...
     * {@link CommandManager#executeExecuteScript(String, CommandManager)}
     * @param commandManager
     * @param args
     * @throws IOException
     * @throws ParaInapproException
     */
    public void execute(CommandManager commandManager,String args[]) throws IOException,ParaInapproException{
        if(args.length>2||args.length==1){
            throw new ParaInapproException("this command only and must accepts one parameter\n");
        }else {
            commandManager.executeExecuteScript(args[1],commandManager);
        }
    }
}
